...
export default function Portfolio() {
  ...
  <p className="text-xl mt-4 text-gray-400 dark:text-black">
    A Full-Stack Developer crafting seamless experiences with modern technologies.
      ...
      <div className="relative w-full max-w-3xl">
        {/* Vertical Line */}
        <motion.div
          className="absolute left-1/2 w-1 dark:bg-gray-900 bg-gray-700 h-full -translate-x-1/2"
          initial={{ height: 0 }}
          animate={{ height: "100%" }}
        ...
        {milestones.map((milestone, index) => (
          ...
          >
            ...
            <motion.div
              className={`card bg-gray  shadow-xl border-2 border-green-200 dark:border-base-300 rounded-lg p-4 dark:bg-white bg-black p-6 rounded-lg shadow-lg w-[80%] md:w-[40%] transition-transform transform hover:scale-105 ${milestone.position === "right" ? "ml-auto text-right mr-6" : "mr-auto text-left ml-6"}`}
              
            >
              <h3 className="text-lg font-semibold bg-green-500 bg-gradient-to-r dark:from-violet-500 dark:to-violet-500 text-transparent bg-clip-text">{milestone.title}</h3>
              <p className="text-gray-300 dark:text-black text-sm mt-2">{milestone.description}</p>
              ...
              {milestone.title === "🎮 Resume" && (
                <motion.div
                  className="mt-4 w-full h-100 bg-gray-800 rounded-lg overflow-hidden"
                  initial={{ opacity: 5 }}
                  whileHover={{ opacity: 1 }}
                  transition={{ duration: 0.3 }}
                >
      ...
      <section className="mt-16 text-center relative z-10">
        <h2 className="text-3xl font-bold bg-gradient-to-r from-teal-400 to-blue-500 dark:from-orange-500 dark:to-yellow-400 text-transparent bg-clip-text">Connect With Me</h2>
        <div className="flex justify-center gap-10 mt-6 text-4xl dark:text-black">
